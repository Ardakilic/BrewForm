...
- [x] 1.1 In `apps/api/src/modules/collection/index.ts`, delete the line `security: [],`
  (line 120, inside the `describeRoute({...})` block for `GET /public`). The surrounding
  `description` and `parameters` keys are unchanged.

**Section verification:**

- [x] 1.2 `make check-api` passes.
- [x] 1.3 `make lint` passes.
- [x] 1.4 `make test-specific filter=apps/api/src/routes/openapi.coverage.test.ts` passes —
  the OpenAPI introspection coverage test confirms the route is still documented.
- [x] 1.5 Grep gate: `rg "security: \[\]" apps/api/src/` returns zero matches.
...
- [x] 2.1 Rename via `git mv` (preserves history):
  ```
...
- [x] 2.2 Search for any import references to the old filenames:
...
- [x] 2.3 In `AGENTS.md`, in the "Testing" section, add after the existing "Tests run with
  `--no-check`" line:
  ```markdown
  - Test files use `*.test.ts` (or `*.test.tsx`) naming — never `*_test.ts`.
  ```

- [x] 2.4 In `openspec/specs/cursor-pagination/spec.md`, update the stale filename reference
...
- [x] 2.6 `make lint` passes.
- [x] 2.7 `make test` passes — all renamed tests are still discovered and pass.
- [x] 2.8 Grep gate: `find apps packages -name '*_test.ts' -o -name '*_test.tsx'` returns
  zero files.
...
- [x] 3.1 Read `apps/api/src/modules/recipe/model.ts` to get the exact signatures of these 6
...
- [x] 3.2 In `apps/api/src/modules/recipe/service.ts`, add 6 exported thin-wrapper functions
...
- [x] 3.3 In `apps/api/src/modules/recipe/index.ts`, replace all 7 `model.*` call sites with
  `service.*`:
...
- [x] 3.4 After replacing all call sites, check if `model` is still referenced anywhere in
...
- [x] 3.5 In `AGENTS.md`, in the "API module pattern" section, add after the existing
  convention bullets:
  ```markdown
...
- [x] 3.6 Read `apps/api/src/modules/follow/service.ts` in full. Identify the exact line
  where the direct `db.select().from(users)` query runs and how the result is used downstream
  (which fields are read from the row).

- [x] 3.7 In `apps/api/src/modules/follow/service.ts`:
...
- [x] 3.8 Read `apps/api/src/modules/badge/service.ts` in full. Identify the exact cursor-batch
  query and how the result is used downstream (the existing code likely maps rows to IDs).

- [x] 3.9 Read `apps/api/src/modules/user/model.ts` to understand the existing imports (`db`,
  drizzle helpers, `users` table) and the file's code style.

- [x] 3.10 In `apps/api/src/modules/user/model.ts`, add a new exported function. Use the same
...
- [x] 3.11 In `apps/api/src/modules/badge/service.ts`:
  - Add import: `import * as userModel from '../user/model.ts';`
...
- [x] 3.12 `make check` passes.
- [x] 3.13 `make lint` passes.
- [x] 3.14 `make test-api` passes — existing tests cover the affected routes/services.
- [x] 3.15 Grep gate: `rg "from 'drizzle-orm'" apps/api/src/modules/*/service.ts` returns
  zero matches (services never import drizzle-orm directly).
- [x] 3.16 Grep gate: `rg "from '@brewform/db'" apps/api/src/modules/*/service.ts` — any
...
- [x] 4.1 Read `apps/api/src/modules/recipe/model.ts` lines 880-910 to see the current
...
- [x] 4.2 Rewrite `buildCursorWhere` (lines ~885-904). Keep the existing cursor validation
...
- [x] 4.3 After the rewrite, check if `or` is still used elsewhere in `model.ts`:
...
- [x] 4.4 In `openspec/specs/lint-style/spec.md`, add a new row to the raw-SQL
...
- [x] 4.5 In `openspec/specs/cursor-pagination/spec.md`, update the code examples in the
...
- [x] 4.6 `make check-api` passes.
- [x] 4.7 `make lint` passes.
- [x] 4.8 `make test-api` passes — cursor pagination tests confirm identical behaviour.
- [x] 4.9 Grep gate: `` rg "sql`" apps/api/src/modules/recipe/model.ts `` — every match
  corresponds to a registry entry (row-value keyset + any pre-existing accepted exceptions
  like atomic counters).
- [x] 4.10 (Manual, optional) On a populated database, run:
  ```sql
...
- [x] 5.1 Run full verification: `make check && make lint && make fmt && make test`.
- [x] 5.2 In `plans/D99-debts.md`, mark D99.4, D99.8, D99.17, and D99.18 as **Resolved**
...
- [x] 5.4 In `plans/TECHNICAL_DEBT.md`, add resolution log entries for D99.4, D99.8, D99.17,
  D99.18 following the existing per-item format.
- [x] 5.5 Archive: `openspec archive remaining-debt-clearance` (after PR merges).
