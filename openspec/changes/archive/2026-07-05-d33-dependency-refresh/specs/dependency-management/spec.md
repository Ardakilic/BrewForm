...
BrewForm's hybrid Deno + npm workspace keeps its dependencies current: manifest `^` floors and the
single root `deno.lock` reflect each dependency's latest compatible release, the lockfile is
frozen-installable via `deno ci`, and the deployment surface (the versions pinned by the Coolify
plan and the `deployment-guide` spec) is decoupled from routine application-dependency bumps. This
capability also records the two majors adopted in this change (`react-router` v8, `zod-openapi` v6)
and the deliberate `drizzle-kit` `0.31`-line pin.
