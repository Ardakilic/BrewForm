...
Recipe write-path layering and the `createRecipeWithRelations` model helper.
The service layer (`apps/api/src/modules/recipe/service.ts`) SHALL NOT import
from `drizzle-orm` or `@brewform/db/schema`; recipe creation with its first
version and all child relations SHALL be encapsulated in a single
`db.transaction` owned by `model.createRecipeWithRelations`, which SHALL
return the full relational shape (matching `findById`). The
`POST /api/v1/recipes` route SHALL be fully documented with a
`RecipeDetailOutputSchema` response.
