...
**Goals:**
- Enable multi-select taste note filtering with AND logic on the recipe list page
- Verify equipment filter correctness (grouping by type, UUID validation, error handling)
- Reposition the clear filters button to the top of the filter sidebar
- Fix theme switcher to display translated labels instead of raw values
- Restyle footer language selector with Base UI Select for visual consistency
- Simplify share section layout by removing the redundant URL textbox
- Add responsive filter sidebar toggle for mobile viewports
- Maintain 1:1 i18n key parity between English and Turkish
- Provide test coverage via property-based tests (fast-check) and example-based unit tests

**Non-Goals:**
- No new custom multi-select component (use Base UI Select `multiple` prop)
- No new components for equipment filters (native select remains, only verification)
- No database schema changes (existing junction tables support the query pattern)
- No changes to authentication, authorization, or user management flows
- No changes to recipe creation, editing, or deletion flows

## Decisions

1. **Base UI Select for multi-select**
   - *Rationale*: The `multiple` prop on `Select.Root` provides built-in multi-selection with checkbox indicators. No need for a custom multi-select implementation.
   - *Alternatives considered*: Custom dropdown with checkboxes (rejected: more code, less accessible)

2. **API parameter change: `tasteNoteId` → `tasteNoteIds`**
   - *Rationale*: The existing single-value `tasteNoteId` parameter is replaced with a comma-separated `tasteNoteIds` parameter (max 10 UUIDs). The API applies AND logic via intersecting subqueries.
   - *Backward compatibility*: Keep `tasteNoteId` as deprecated fallback

3. **No new components for equipment filters**
   - *Rationale*: Equipment filters remain as native `<select>` elements since they are single-select and already work correctly. Only verification and minor correctness fixes are needed.

4. **URL as source of truth for filter state**
   - *Rationale*: All filter state lives in URL search params, making filtered views shareable and bookmarkable.

5. **Property-based testing with fast-check**
   - *Rationale*: Universal correctness properties (hierarchy rendering, AND logic, URL round-trip, trigger labels, equipment grouping, UUID validation, clear button visibility, translated labels, key parity) are verified with minimum 100 iterations each.

## Risks / Trade-offs

- [Risk] **Multi-select performance with many taste notes** → Mitigation: Cap at 10 selections enforced in UI and API schema
- [Risk] **AND logic may return empty results for broad taste note combinations** → Mitigation: Clear empty-state message; users can deselect individually
- [Risk] **Clipboard API unavailable in some environments** → Mitigation: Error state display for 3 seconds with graceful fallback
- [Risk] **Base UI Select accessibility on mobile** → Mitigation: Minimum 44×44px tap targets, ARIA attributes, screen reader testing
- [Trade-off] **Property tests add CI time** → Acceptable: fast-check runs are fast; correctness guarantees justify the cost
