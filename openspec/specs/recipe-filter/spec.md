...
Each branch SHALL generate Drizzle conditions using `ilike`, `inArray`, `gte`, `lte`, `or`, `and` helpers — no raw SQL. Both `listRecipes()` (in `service.ts`, via `model.buildListRecipesWhere`) and `findStarred()` (in `model.ts`) SHALL call this helper and SHALL NOT contain any inline filter `if` blocks.
