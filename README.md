...
Serena runs as a Docker service with the full monorepo mounted as a single workspace at
`/workspace/brewform`.
...
make serena-index   # Force re-index the workspace
...
> **Tip:** The index cache persists in `.serena/cache/` on the host (via bind mount). After pulling
> new code or making structural changes, run `make serena-index` to keep the index fresh.
