    ...
    <div className='print-container mx-auto max-w-2xl px-6 py-8'>
      ...
      <table className='w-full mb-4 text-sm'>
        <tbody>
          {v.brewMethod && (
            <tr>
              <td className='py-1 font-medium pr-4'>Brew Method</td>
              <td>{v.brewMethod.replace(/_/g, ' ')}</td>
            </tr>
          )}
          {v.drinkType && (
            <tr>
              <td className='py-1 font-medium pr-4'>Drink Type</td>
              <td>{v.drinkType.replace(/_/g, ' ')}</td>
            </tr>
          )}
          {v.productName && (
            <tr>
              <td className='py-1 font-medium pr-4'>Product Name</td>
              <td>{v.productName}</td>
            </tr>
          )}
          {v.coffeeBrand && (
            <tr>
              <td className='py-1 font-medium pr-4'>Coffee Brand</td>
              <td>{v.coffeeBrand}</td>
            </tr>
          )}
          {v.coffeeProcessing && (
            <tr>
              <td className='py-1 font-medium pr-4'>Processing</td>
              <td>{v.coffeeProcessing}</td>
            </tr>
          )}
          {v.grinder && (
            <tr>
              <td className='py-1 font-medium pr-4'>Grinder</td>
              <td>{v.grinder}</td>
            </tr>
          )}
          {v.grindSize && (
            <tr>
              <td className='py-1 font-medium pr-4'>Grind Size</td>
              <td>{v.grindSize}</td>
            </tr>
          )}
          {v.groundWeightGrams && (
            <tr>
              <td className='py-1 font-medium pr-4'>Dose</td>
              <td>{v.groundWeightGrams}g</td>
            </tr>
          )}
          {v.extractionTimeSeconds && (
            <tr>
              <td className='py-1 font-medium pr-4'>Time</td>
              <td>{v.extractionTimeSeconds}s</td>
            </tr>
          )}
          {v.extractionVolumeMl && (
            <tr>
              <td className='py-1 font-medium pr-4'>Yield</td>
              <td>{v.extractionVolumeMl}ml</td>
            </tr>
          )}
          {v.temperatureCelsius && (
            <tr>
              <td className='py-1 font-medium pr-4'>Temperature</td>
              <td>{v.temperatureCelsius}°C</td>
            </tr>
          )}
          {v.brewRatio && (
            <tr>
              <td className='py-1 font-medium pr-4'>Ratio</td>
              <td>1:{v.brewRatio}</td>
            </tr>
          )}
          {v.rating && (
            <tr>
              <td className='py-1 font-medium pr-4'>Rating</td>
              <td>{v.rating}/10 {emoji ? emoji.emoji : ''}</td>
            </tr>
          )}
        </tbody>
      </table>
