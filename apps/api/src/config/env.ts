...
const envSchema = z.object({
  ...
  JWT_ACCESS_EXPIRY: z.string().regex(
    /^\d+[smhdM]$/,
    'Invalid JWT_ACCESS_EXPIRY format; expected like 30d, 12h',
  ).default('15m'),
  JWT_REFRESH_EXPIRY: z.string().regex(
    /^\d+[smhdM]$/,
    'Invalid JWT_REFRESH_EXPIRY format; expected like 30d, 12h',
  ).default('7d'),
  JWT_REMEMBER_ME_EXPIRY: z.string().regex(
    /^\d+[smhdM]$/,
    'Invalid JWT_REMEMBER_ME_EXPIRY format; expected like 30d, 12h',
  ).default('180d'),
