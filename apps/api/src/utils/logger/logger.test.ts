...
import { getLogger, logAudit, logSecurity } from "./index.ts";
...
describe("Logger Utilities", () => {
  ...
  describe("logAudit", () => {
    it("should be callable without errors", () => {
      expect(() =>
        logAudit("recipe_created", "recipe", "recipe_123", "user_456")
      ).not.toThrow();
    });
  });
  ...
  describe("logSecurity", () => {
    it("should be callable without errors", () => {
      expect(() => logSecurity("login_failed", { email: "test@example.com" }))
        .not.toThrow();
    });
  });
});
