...
export const CacheKeys = {
  ...
  userByUsername: (username: string): CacheKey => ['user', 'username', username] as const,

  // Recipes
  recipe: (id: string): CacheKey => ['recipe', id] as const,
  recipeBySlug: (slug: string): CacheKey => ['recipe', 'slug', slug] as const,
  recipeList: (filterHash: string): CacheKey => ['recipes', 'list', filterHash] as const,
  latestRecipes: (limit?: number): CacheKey =>
    limit !== undefined ? ['recipes', 'latest', limit] as const : ['recipes', 'latest'] as const,
  popularRecipes: (limit?: number): CacheKey =>
    limit !== undefined ? ['recipes', 'popular', limit] as const : ['recipes', 'popular'] as const,
