...
>
> **Note:** The body below this line (including the 2026-07-13 banner) is the pre-refresh draft, kept as historical context. Treat the corrections in this block as authoritative; do not follow the body where they conflict.
>
> - Still unshipped: no `recipeTemplates` table in packages/db/src/schema.ts, no `apps/api/src/modules/template/`, no `/templates` mount in apps/api/src/routes/index.ts, no `recipe-template.ts` in packages/shared/src/schemas/, and no openspec spec or change covers templates. The entire feature is net-new; no scope was shipped elsewhere (F01 collections, F04/F05 notifications, F08 compare/merge, F09 version diff, F11 search, D27/D12/D23/D42 do not touch template scope).
> - Route registration (body ~457–462): `apps/api/src/index.ts` still does not exist (entrypoint is main.ts). Register via `routes.route('/api/v1/templates', template)` in apps/api/src/routes/index.ts (module mounts at :46–66).
> - Hardcoded `BrewMethodEnum`/`DrinkTypeEnum` z.enum lists (body ~100–111): the values happen to match today, but they duplicate the source of truth — use `z.enum(BREW_METHOD_VALUES)` / `z.enum(DRINK_TYPE_VALUES)` from `@brewform/shared/constants` (constants/index.ts:29–30); the DB pgEnums are built from the same tuples (schema.ts:50,52).
> - OpenAPI bar is stricter than the body: every route needs `describeRoute()` responses wrapped in `resolver(successEnvelope(XOutputSchema))` / `resolver(paginatedEnvelope(...))` / `resolver(ErrorEnvelopeSchema)`, request bodies via `jsonRequestBody()` (apps/api/src/utils/openapi/index.ts:18), a `Templates` tag registered in the tags array at apps/api/src/routes/openapi.ts:59–92, and coverage is enforced by apps/api/src/routes/openapi.coverage.test.ts. The body's bare `{ description }` responses are insufficient — copy the collection module (apps/api/src/modules/collection/index.ts:44–112) as the most recent reference.
> - D42 typed web boundary: `templateApi` in apps/web/src/api/index.ts must be typed from shared schemas. Add `TemplateOutputSchema` under packages/shared/src/schemas/responses/, export it from responses/index.ts and schemas/index.ts (mirror the collection exports, schemas/index.ts:150–159), and type the list as `api.getWithMeta<PaginatedResponse<TemplateOutput>>` (see collectionApi, apps/web/src/api/index.ts:269–299). apps/web/src/api/types.ts is deleted.
> - i18n (body omits it): template UI strings go in packages/shared/src/i18n/en.json and tr.json — key parity is enforced by packages/shared/src/i18n/i18n.test.ts:7.
> - Seed idempotency (body ~519–610): seed.ts requires `onConflictDoNothing` keyed on a unique constraint, or select-and-reuse; the proposed `recipe_template` table has no unique constraint on `name`, so add one or select-and-reuse by name+isSystem. seed.ts is guarded with `if (import.meta.main)` (packages/db/src/seed.ts:973) — add a helper function, not a second entrypoint.
> - Internal inconsistency in the body: service.ts calls `model.findManyWithFilters(filters, userId)` (body ~253) but model.ts only defines `findMany(where, page, perPage)` (body ~200); `TemplateFilterSchema.search` (body ~136) also has no model implementation. Reconcile before implementing.
> - Logging: the body contains no `createLogger` anywhere — AGENTS.md requires a module-scoped logger (API: `./utils/logger/index.ts`; web: `@/utils/logger.ts`), entry/exit debug logs on service functions, and mount/unmount logs on pages.
> - Verified still accurate (line refs refreshed): `recipeVersions` columns the template mirrors are at schema.ts:202–265 (not 191–212); `computeBrewRatio` at packages/shared/src/utils/metrics.ts:2, re-exported via utils/index.ts:19; RecipeCreatePage `useSearchParams` + setters incl. `setBrewerDetails` at RecipeCreatePage.tsx:31,35–57,146; `authMiddleware`/`optionalAuthMiddleware` at apps/api/src/middleware/auth.ts:58,117; offset `paginated()` still valid for the list (apps/api/src/utils/response/index.ts:39–58) — cursor pagination (D27, `cursorPaginated()` :72–91) exists but is optional here; extend `usersRelations` at schema.ts:1024.
> - Conventions check: the body's schema proposal already avoids JSONB/UUID columns, uses varchar(36) ids, soft deletes, and the model→service→index split — no conflicts there. No raw SQL anywhere.
