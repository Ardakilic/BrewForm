...
>
> **Note:** The body below this line is the pre-refresh draft, kept as historical context; it includes an older 2026-07-13 banner whose line numbers have since drifted and which this banner supersedes. Treat the corrections below as authoritative — do not use the body as a current implementation reference.
>
> - **Already shipped (overlap):** `/admin/stats` (apps/api/src/modules/admin/index.ts:56), `/analytics/users` (:70), `/analytics/recipes` (:86), `/analytics/top-recipes` (:102), `/analytics/top-users` (:118), guarded by router-level `admin.use('*', authGuard, adminGuard)` (:53). Model: `getDashboardStats` (apps/api/src/modules/admin/model.ts:480), `getUserGrowth` (:518), `getRecipeGrowth` (:529), `getTopRecipes` (:540), `getTopUsers` (:548); service wrappers at apps/api/src/modules/admin/service.ts:536-593. **Net-new:** only `/analytics/trends`, `/analytics/retention`, `/analytics/export` — extend the existing functions, don't duplicate them.
> - **Raw-SQL conflict (still the main issue):** the plan's `to_char(...)` GROUP BY violates "No raw SQL, no Postgres-specific operators". The established pattern (`getUserGrowth`, model.ts:518-526) selects raw `createdAt` rows with `gte(createdAt, since)` and buckets by date in JS — keep `fillMissingDates` and feed it those rows. Drizzle `.union()` is also unused anywhere in the API — run the three `selectDistinct` queries and dedupe with a JS `Set`. The brew-method distribution query is fine as-is (plain Drizzle `groupBy(recipeVersions.brewMethod)` + `count()`).
> - **Route snippets stale:** drop per-route `authMiddleware, adminMiddleware` (router-level guard at index.ts:53 covers it), and use `days: z.coerce.number().int().min(1).max(365)` to match the existing analytics endpoints (index.ts:78), not `period: z.enum(['7d','30d','90d'])`.
> - **OpenAPI insufficient:** bare `responses: { 200: { description } }` no longer passes convention — new routes need `resolver(successEnvelope(XOutputSchema))` plus `ErrorEnvelopeSchema` (401) responses. No admin-analytics output schemas exist yet in packages/shared/src/schemas/responses/ — add them there (with co-located unit test). The `Admin` tag is already registered (apps/api/src/routes/openapi.ts:63). The CSV route documents `text/csv` and is NOT envelope-wrapped.
> - **Frontend data fetching:** no TanStack Query anywhere in apps/web; `api.get` always parses JSON (apps/web/src/api/client.ts:64), so `responseType: 'blob'` (axios-style) doesn't exist — CSV download needs a raw `fetch()` with `credentials: 'include'`. AdminDashboard (apps/web/src/pages/admin/AdminDashboard.tsx:21) fetches via `useEffect` + `api.get('/admin/stats')` (:34) — mirror that pattern (a few pages use `useLoaderData`, but this page doesn't). Route: apps/web/src/router.tsx:340.
> - **Types:** `apps/web/src/api/types.ts` was DELETED (D42) — define AnalyticsTrends/RetentionMetrics as Zod schemas in packages/shared/src/schemas/responses/ and use `z.infer` on the web side.
> - **Components:** apps/web/src/components/admin/ contains only BanDialog.tsx — LineChart/PieChart/PeriodSelector are net-new; no chart library is installed, so the SVG approach stands. AdminDashboard already defines a local `StatCard` (AdminDashboard.tsx:70) — reuse/extract it rather than creating a duplicate file.
> - **Schema facts verified:** `comments.authorId` (packages/db/src/schema.ts:602), `recipes.authorId` (:136), `userRecipeLikes.userId` (:692, table at :688), `userRecipeRatings.userId` (:709, table at :705) — likes/ratings have NO `deletedAt` (plan correctly omits it); users/recipes/comments must filter `deletedAt IS NULL` (plan does). `recipeVersions.brewMethod` (:216), `recipeVersions.createdAt` (:238). admin/model.ts does not yet import `userRecipeLikes`/`userRecipeRatings` — add them to the `@brewform/db/schema` import (model.ts:13-29).
> - **Cache (optional):** existing analytics service functions don't cache. If added, use `CacheProvider` (`get/set(key: string[], { ttlMs })`, apps/api/src/utils/cache/index.ts:9-18) via the `cacheProvider` singleton already imported at admin/index.ts:33 — never `Deno.openKv()` directly.
> - **Tests:** add to the existing apps/api/src/modules/admin/model.test.ts / service.test.ts / index.test.ts (`*.test.ts`, bdd + @std/expect); the openapi coverage test (apps/api/src/routes/openapi.coverage.test.ts) will pick up new routes.
> - **Stale references:** the body's References (model.ts:453 `getDashboardStats`, :491 `getUserGrowth`) are now :480 and :518.
> - **Pre-existing risk to reconcile:** AdminDashboard's local `DashboardStats` interface (AdminDashboard.tsx:11-18) expects `totalPhotos/recentSignups/recentRecipes`, but `GET /admin/stats` returns `totalReports/pendingReports/newUsersToday/newRecipesToday` (model.ts:506-514). Fix this mismatch while reworking the dashboard.
