...
> **Note:** The body below this line is the original pre-implementation draft plan, kept as historical context. It contains obsolete guidance (grinder wiring, UUID cursors, raw SQL, RangeSlider, inline filters) that was superseded by the OpenSpec change `f11-advanced-search` design and specs. Do not use the body as a current implementation reference — see `openspec/specs/advanced-search/` for the shipped contract.
