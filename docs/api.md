...
`portafilter`, `basket`, `puck_screen`, `paper_filter`, `tamper`, `gooseneck_kettle`, `mesh_filter`, `cezve`, `scale`, `thermometer`, `other`
...
Returns the image directly with the appropriate `Content-Type` header. The QR code encodes the recipe's public URL.
