...
- **Deno test imports use `jsr:` specifiers**: No import maps. Use `jsr:@std/testing/bdd` for `describe`/`it`, `jsr:@std/expect` for `expect`, `jsr:@std/assert` for `assertEquals`/`assertThrows`. These are separate JSR packages — `expect` is NOT exported from `@std/testing/bdd`.
- **Deno test requires `--unstable-sloppy-imports`**: Because shared package barrel files omit `.ts` extensions (for tsc), Deno's module resolver needs `--unstable-sloppy-imports` to resolve bare specifiers like `./conversion` → `./conversion.ts`.
- **Deno test requires `--no-check`**: Running tests with `--no-check` skips slow type checking; type correctness is verified separately via `make check`. The test runner also needs `--allow-env --allow-read --allow-write --allow-net --allow-sys` permissions.
- **Pino logger requires `--allow-sys`**: Pino calls `os.hostname()` which needs `--allow-sys` in Deno. Logger tests should test module shape without actually running pino in the test.
...
- **Documentation split**: Feature docs (auth, recipes, taste-notes) go in `docs/` for user-facing reference. JSDoc comments in code for developer reference. Docs explain "why" and "how"; JSDoc explains "what". No duplication.
- **`make check` requires npm install + prisma generate**: The `make check` command runs `npm install` and `cd packages/db && npx prisma generate` before `deno check`. If Deno can't find nodemailer or @prisma/client, run `make install && make db-generate` first.